<script>
	 ...
	 */
	export let tailwindVersion = ""; // Can be v4, v3.3+, v3.2 or v2
...
<header class="p-4 mt-1 -ml-4 -mr-4 bg-cyan-800 sm:rounded">
	<h1 class="leading-none text-cyan-50">
		Find the nearest Tailwind {tailwindVersion !== "v4" ? tailwindVersion : ""} colour
	</h1>
	...
	<p class="mt-2 text-cyan-100">
		...
		{#if tailwindVersion === "v4"}
			<a href="https://tailwindcss.com/docs/colors"> Tailwind colour palette</a>
		{/if}

		{#if tailwindVersion === "v3.3+"}
			<a href="https://v3.tailwindcss.com/docs/customizing-colors">
		...
		<details class="select-none">
			...
			<div class="flex flex-col sm:flex-row mt-2 gap-y-2 gap-x-4 sm:items-center justify-start">
				<a
					...
					class="no-underline hover:underline w-fit px-2">Tailwind v4</a
				>

				<a
					href="/3-3"
					class:active={$page.url.pathname === "/3-3"}
